...
| `docs/assets/tree.svg` | 树，浅色配色 |
| `docs/assets/tree-dark.svg` | 树，深色配色 |
| `docs/assets/board.svg` | 看板 |
| `docs/plan.html` | 一张可以点击的页面 |

图里会动的东西都携带信息：虚线沿着通往"进行中"任务的连线流动；图标在推进中的任务上缓慢旋转，在被阻塞的
任务上呼吸般脉动——那是唯一值得打断读者的状态；进度条在加载时增长一次；卡片依次淡入，而不是一齐出现。
本文件顶部的卡片带则会一直滚动。其余一切保持静止。

如果读者的系统要求减少动效，得到的就是一张静止的图。打印时动效会被完全关掉：否则停在淡入第一帧的
渲染器会印出一张空白卡片。

### 两种图都做不到的一件事

Mermaid 会忽略 `click`，作为图片提供的 SVG 处于沙箱中——两者都无法承载链接。所以有了第三个后端：把输出
文件命名为 `.html`，每个指向某处的任务就**变成**通往其 PR、issue 或分支的链接。页面上同时有树视图和看板、
按状态/负责人/标签的筛选，备注则以悬停提示呈现。

这是一个自包含的文件：没有脚本、没有字体、没有图片——整个交互都由 CSS 的 `:has()` 实现。示例见
[docs/plan.html](../plan.html)，细节见 [docs/html-export.md](../html-export.md)。
