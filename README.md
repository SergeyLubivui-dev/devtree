...
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/assets/editor-dark.svg">
  <img alt="The editor: a rail of sections, the plan as a list, and the same plan drawn beside it" src="docs/assets/editor.svg" width="800">
</picture>
...
adds work underneath it — that green `+` above is the real affordance, not a decoration. Five views
behind a dropdown — tree, board, page, Mermaid, YAML — rendered on demand, with nothing written to
disk until you press **Write outputs**.

That picture is not a screenshot. It is drawn by devtree, from the same components its diagrams are,
so it cannot go stale the way a screenshot does the week after somebody changes a colour.
...
## What the diagrams are made of

Every picture in this repository — the plan diagrams, the board, the illustration above — is
assembled from one vocabulary of twenty components.
...
| **Pill** a state, in a word | **Avatar** an owner as initials | **Milestone** a point, not a span | **Legend** the key to a drawing |
...
The gallery above is generated by walking the component list, so a component added without a demo
fails the build rather than quietly going undocumented. More:
